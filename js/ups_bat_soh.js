...
channelKeys.push({channelNumber:localStorage.getItem("channel"), name:'Temperature',key:localStorage.getItem("apikey"),  
   //fieldList:[{field:1,axis:'O'},{field:2,axis:'T'}]});  
     fieldList:[{field:4,axis:'T'}]});
...
{  
 ...
 function loadThingSpeakChannel(sentChannelIndex,channelNumber,key,sentFieldList) {  
   ...
   // get the Channel data with a webservice call  
 	 $.getJSON('https://api.thingspeak.com/channels/'+channelNumber+'/feeds.json?api_key='+key+'&results=2500', function(data)    
   {  
	   // if no access  
	   if (data == '-1') {  
 ...
 function createChart() {  
             ...
             {  
              ...
              {    
               ...
               {  
				...
				$.getJSON('https://api.thingspeak.com/channels/'+channelKeys[channelIndex].channelNumber+'/feeds.json?api_key='+channelKeys[channelIndex].key+'&results=1', function(data)    
                {   
  ...
  {  
	...
	chartOptions.xAxis.title.text = 'Date\Time';  
     ...
     {  
       ...
       {  
         ...
         var fieldStr = "data.feeds["+h+"].field"+fieldList[fieldIndex].field;  
			...
			//document.getElementById("COVal").innerHTML = parseFloat(p[1]).toFixed(3);
	 	  	// if a numerical value exists add it  
	   		if (!isNaN(parseInt(v))) { fieldList[fieldIndex].data.push(p); }  
       }  
